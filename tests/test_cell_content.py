...
class TestCellContent:
    ...
    def test_single_paragraph_cell(self, table_doc):
        ...
        # 提取文本
        dn_texts = [block.text for block in dn_cell.blocks() if isinstance(block, Paragraph)]
        dn_text = "\n".join(dn_texts)
    ...
    def test_multi_paragraph_cell(self, multiline_cell_doc):
        ...
        # docxnote 段落数
        dn_paragraphs = [block for block in dn_cell.blocks() if isinstance(block, Paragraph)]
